...
class qtype_formulas_question extends question_graded_automatically_with_countback
	...
	public $numpart; /* number of formulas_parts */
     ...
     * When there is a separated unit answer field for part i, it is called "i_n" where n is the number of coordinates for part i
	 * Sor for instance if subquestion 2 has 3 coordinates and a separate unit field we will have 2_0, 2_1, 2_2, 2_3 (last one is unit)
	 * When there is a combined unit answer field for part i, it is called "i_"
     */
	...
	public function start_attempt(question_attempt_step $step, $variant) {
        try {
			...
			$this->globalvars_text = $this->varsglobal;    // TODO is it usefull ?
            $step->set_qt_var('_randomsvars_text', $this->randomsvars_text);
            $step->set_qt_var('_globalvars_text', $this->varsglobal);
            return true;    // success
        } catch (Exception $e) {
            return false;   // fail
        }
    ...
    public function apply_attempt_state(question_attempt_step $step) {
		...
		$this->varsglobal = $step->get_qt_var('_globalvars_text');
     ...
     * and apply format_text() to general feedback
     *
    ...
    public function format_generalfeedback($qa) {
        ...
        return $this->format_text($this->qv->substitute_variables_in_text($globalvars, $this->generalfeedback),
    ...
    public function get_num_parts_right(array $response) {
        $this->rationalize_responses($response);      // may throw if the subqtext changed
        $checkunit = new answer_unit_conversion;
        $c = 0;
    ...
    public function get_correct_response() {
        ...
        foreach ($this->parts as $index => $part) {
            ...
            if ($tmp === null)  return array();      // TODO : really examine what to return in that case empty array or null ?
            if ($part->part_has_combined_unit_field()) {
                $tmp["{$index}_"] = $tmp["{$index}_0"] . $tmp["{$index}_1"];
                unset($tmp["{$index}_0"], $tmp["{$index}_1"]);
            } else if (!$part->part_has_separate_unit_field()) {
                unset($tmp["{$index}_{$part->numbox}"]);
            }
    ...
    // TODO verify it works in all cases : combined and separate unit field, no unit field
		$complete = true;
		foreach ($this->parts as $i => $part)  {
            if ($part->part_has_combined_unit_field()) {
                ...
                if ($part->part_has_separate_unit_field()) $complete = $complete && array_key_exists("${i}_{$part->numbox}", $response) && $response["${i}_{$part->numbox}"] != '';
            }
        }
		return $complete;
	}
    ...
    public function is_same_response_for_part($index, array $prevresponse, array $newresponse) {
        return $this->parts[$index]->part_is_same_response($prevresponse, $newresponse);
    }
...
// TODO if student response is invalid decide what to do
		foreach ($this->parts as $i => $part)  {
			...
			if (!empty($response["${i}_"])) return true;
        }
        return false;
	}
    ...
    public function grade_response(array $response) {
        try {
            ...
            foreach ($this->parts as $idx => $part) {
                list($this->anscorrs[$idx],$this->unitcorrs[$idx]) = $this->grade_responses_individually($part, $response, $checkunit);
    ...
    function get_correct_responses_individually($part) {
        try {
            ...
            $localvars = $this->get_local_variables($part);
            if (is_string($res[0]))  $res = $this->qv->substitute_partial_formula($localvars, $res);
    ...
    function grade_responses_individually($part, $response, &$checkunit) {
        ...
        $vars = $this->get_local_variables($part);     // it contains the global and local variables before answer
    ...
    function rationalize_responses_for_part($index, array &$response) {
//        echo "rationnalise part $index";var_dump($response);
        $part = $this->parts[$index];
        foreach (range(0,$part->numbox) as $j) {
            $name = "{$index}_$j";
            $response[$name] = isset($response[$name]) ? trim($response[$name]) : '';   // fill the missing responses by an empty string
            if (strlen($response[$name]) > 128)  $response[$name] = substr($response[$name], 0, 128);    // restrict length to 128
            if (isset($response["{$index}_"])) {   // for a long answer box, always parse it into a number and unit, say, "0_0" and "0_1"
                $response["{$index}_"] = substr(trim($response["{$index}_"]), 0, 128);
                $tmp = $this->qv->split_formula_unit($response["{$index}_"]);
                $response["{$index}_0"] = $tmp[0]; // It will be checked later whether tmp[0] is a number
                $response["{$index}_1"] = isset($tmp[1]) ? $tmp[1] : '';
...
//        echo "after";var_dump($response);
    }

    function rationalize_responses(array &$response) {
        foreach ($this->parts as $i => $part)  foreach (range(0,$part->numbox) as $j) {
            $name = "${i}_$j";
            $response[$name] = isset($response[$name]) ? trim($response[$name]) : '';   // fill the missing responses by an empty string
            if (strlen($response[$name]) > 128)  $response[$name] = substr($response[$name], 0, 128);    // restrict length to 128
    ...
    //   var_dump($this->randomsvars_text);
	    $this->randomsvars = $this->qv->evaluate_assignments($this->qv->vstack_create(), $this->randomsvars_text);
        if (!isset($this->globalvars)) // Perform lazy evaluation, when the global variable does not exist before
    ...
    function get_local_variables($part) {
        if (!isset($this->localvars[$part->location])) // Perform lazy evaluation, when the local variable does not exist before
...
// echo "coucou3 ";
            ...
            $this->rationalize_responses_for_part($index, $response);
...
//        var_dump($partresults);
    ...
    public function get_parts_and_weights() {
        $weights = array();
        foreach ($this->parts as $index => $part) {
            $weights[$index] = $part->answermark / $this->defaultmark;
        }
//        var_dump($weights);
        return $weights;
    }
    ...
    function get_evaluated_answer($part) {
        if (!isset($this->evaluatedanswer[$part->location])) {   // Perform lazy evaluation
            ...
            $this->evaluatedanswer[$part->location] = $res->type[0]=='l' ? $res->value : array($res->value); // convert to array of numbers
        ...
        return $this->evaluatedanswer[$part->location]; // no type information needed, it returns array of number or string
    }
    ...
    public function check_file_access($qa, $options, $component, $filearea, $args, $forcedownload) {
        ...
        if ($component == 'qtype_formulas' && ($filearea == 'answersubqtext' || $filearea == 'answerfeedback')) {
			...
			for ($i = 0; $i < $numpart; $i++) {
				if ($this->parts[$i]->id == $itemid) return true;
			}
			return false;
...
//               echo " looking response $responseindex ";
//                $partresponse = $part->filter_array($response);
                if ($part->part_is_same_response($lastresponse, $response)) {
//               echo " skipping response $responseindex ";
                    continue;
                }
//                $tries++;
                $lastresponse = $response;
                $lastchange = $responseindex;
//                echo " coucou 3 lastchange = $lastchange ";
                $this->rationalize_responses($response);
//                echo "after rationalize\n";
//                var_dump($response);
                list($anscorrs,$unitcorrs) = $this->grade_responses_individually($part, $response, $checkunit);
//                echo " anscorrs = $anscorrs and unitcorrs = $unitcorrs and unitpenality = ".$part->unitpenalty . " ";
                $partfraction = $anscorrs * ($unitcorrs ? 1 : (1-$part->unitpenalty));
//                echo "partfraction for $responseindex = $partfraction ";
            }
//            echo "\n end of loop for responses and last change for part $index lastchange = $lastchange tries=$tries ";
...
//            echo " end loop for part $index and fractionsum = $fractionsum ";
        }
...
class qtype_formulas_part {
	...
	public function part_has_combined_unit_field() {
        return strlen($this->postunit) != 0 && $this->numbox == 1 && $this->answertype != 1000 && (strpos($this->subqtext, "{_0}{_u}") !== false || (strpos($this->subqtext, "{_0}") === false && strpos($this->subqtext, "{_u}") === false));
    }
    ...
    public function part_get_expected_data()  {
        ...
        if ($this->part_has_combined_unit_field()) {
            ...
            if ($this->part_has_separate_unit_field()) $expected["${i}_{$this->numbox}"] = PARAM_RAW;
        }
        return $expected;
    }
    
    public function part_is_gradable_response(array $response) {
        // TODO and after that use in is_fradable_response
